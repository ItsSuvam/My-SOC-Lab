...
- Wazuh agent + Sysmon - Windows 11 host
...
- Wrote a custom Suricata detection rule (sid:9000001) and validated it end-to-end:
  ...
  1000+ hits confirmed over 24h)
  - Evidence: `evidence/01-wazuh-dashboard-overview.png`, `evidence/02-linux-agent-active.png`,
    `evidence/03-suricata-custom-rule-detected.png`
- Wrote the first investigation playbook (playbooks/icmp-test-alert.md)
- Added a Windows endpoint: Sysmon (SwiftOnSecurity config) + Wazuh agent (windows-endpoint, ID 002)
  - Fixed IPv6 loopback issue: WSL2 mirrored networking resolved `localhost` to `::1`, which the
    manager could not accept connections on — switched `ossec.conf` `<address>` to `127.0.0.1`
    to force IPv4, agent connected successfully
  - Manually registered agent key via `manage_agents` (auto-enrollment on port 1515 failed)
  - Added `<localfile>` block to collect `Microsoft-Windows-Sysmon/Operational` event channel
  - Verified end-to-end: Sysmon -> Wazuh agent -> Manager -> Dashboard, confirmed via
...
- WSL2 mirrored networking resolves `localhost` to IPv6 `::1` by default, which the Wazuh
  manager's remoted service did not accept connections on for the Windows agent - forcing
  IPv4 (`127.0.0.1`) in `ossec.conf` fixed it.
